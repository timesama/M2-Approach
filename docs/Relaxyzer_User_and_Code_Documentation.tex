...
\begin{itemize}[noitemsep]
  ...
  \item \(T_1\) and \(T_2\) relaxation curves in two-column CSV, tab-separated text, and generic numeric two-column files.
...
\end{itemize}
Legacy Stelar \texttt{.sef} files are explicitly rejected by the current \(T_1/T_2\) controller; the user is instructed to convert such data to the supported text/table/CSV formats.
...
\begin{itemize}
  ...
  \item Log-scale radio button: plots the distribution against \(\log_{10}(\TtwoStar_{\mathrm{lin}})\).
...
\begin{equation}
\TtwoStar_{\mathrm{lin}}(\tau_{DQ}) = a\tau_{DQ}+b
\end{equation}
...
\subsection{Step-by-Step User Workflow}
\begin{enumerate}
  \item Produce saved DQ tables from the DQ tab with columns \(\TtwoStar_{\mathrm{lin}}\) and DQ Norm.
...
\begin{equation}
\text{column 4} = \TtwoStar_{\mathrm{lin}}, \qquad \text{column 5} = I_{DQ,\mathrm{norm}}.
\end{equation}
...
\subsection{Purpose and Scope}
The T1T2 tab loads and fits \(T_1\) and \(T_2\) relaxation curves. It supports regular two-column relaxation curves, text files containing T1 curves for different FID regions, and converted FFC/Stelar-style profile data in supported table/text/CSV formats. Current legacy \texttt{.sef} files are rejected.
...
\end{tabular}
\end{center}
The integral-sum feature accepts a comma-delimited CSV with exactly six columns. It uses column 0 as DQ time, column 1 as DQ amplitude, and column 4 as \(\TtwoStar_{\mathrm{lin}}\). This matches the saved DQ table after linearization.
