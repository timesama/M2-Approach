...
\begin{itemize}
  \item Load numeric text, CSV, DAT, TXT, XLSX files in the formats described in this manual.
  \item Process several SE or DQ files in one run.
...
\begin{itemize}
  ...
  \item It does not replace NMR interpretation. Some reported parameters are model-dependent and should be judged accordingly.
...
FID and raw DQ workflows use a three-column numeric time-domain format:
\begin{center}
\begin{tabular}{lll}
\toprule
...
\section{FID / Solid Echo / MSE Workflow}

FID here refers to the Free Induction Decay. It may be recorded using single pulse excitation or any other methods.

\subsection{Purpose}
Use the FID workflow when you have time-domain files containing time, real signal, and imaginary signal, and you want to estimate solid content, \Mtwo{}, \TtwoStar{}, or see the temperature trends.
...
Optional reference/empty files & If using reference (i.e. glycerol signal for magnet inhomogeneity correction) or empty-signal (for baseline correction), select matching files in the same order as the raw files. \\
\bottomrule
\end{longtable}

\subsection{Typical Use}
\begin{enumerate}
  \item Select the raw files for the FID tab.
  \item Adjust settings such as solid-content windows, baseline/empty correction, reference correction, smoothing, and figure export if needed in Settings tab.
  ...
  \item Optionally group data or calculate an activation-energy trend.
  \item Save the table.
\end{enumerate}
...
The FID table is the primary result. It contains the experimental variable, filename information, solid-content value, \Mtwo{}, and \TtwoStar{}. Optional figure export can also write plotted FID/FFT curves and images beside the source data.
...
\begin{enumerate}
  ...
  \item Fit the normalized distribution with Gaussian, Lorentzian, or pseudo-Voigt (weighted sum of Gaussian and Lorentzian functions) model.
...
% todo: i am too laze, and this is too boring.
% add FID build up
% add new functional for Spin Diffusion - BRAND NEW DQSD
% check carefully description for DQMQ, there are many new things
